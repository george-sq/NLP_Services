...
def main():
    ...
    sim_lda_A = indexLda[lda_A]
    sim_lda_B = indexLda[lda_B]
    sim_lda_C = indexLda[lda_C]

    print("A lda相似性：", sorted(enumerate(sim_lda_A), key=lambda item: -item[1])[:5])
    print("B lda相似性：", sorted(enumerate(sim_lda_B), key=lambda item: -item[1])[:5])
    print("C lda相似性：", sorted(enumerate(sim_lda_C), key=lambda item: -item[1])[:5])
