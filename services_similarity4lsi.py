...
def main():
    ...
    indexLsi.save("./Out/IndexOfSimilarity/LSI_11Topic.idx")
    ...
    sim_lsi_A = indexLsi[lsi_A]
    sim_lsi_B = indexLsi[lsi_B]
    sim_lsi_C = indexLsi[lsi_C]

    print("A lsi相似性：", sorted(enumerate(sim_lsi_A), key=lambda item: -item[1])[:5])
    print("B lsi相似性：", sorted(enumerate(sim_lsi_B), key=lambda item: -item[1])[:5])
    print("C lsi相似性：", sorted(enumerate(sim_lsi_C), key=lambda item: -item[1])[:5])
